...
resource "aws_codebuild_project" "cbuild_project_angular" {
  ...
  environment {
    ...
    name  =  "S3_BUCKET"
	  value =  "${join("-",list(var.prefix,"front"))}"
    }
  }
...
resource "aws_codepipeline" "cpipeline_project_angular" {
  name     = "${var.prefix}-front"
  ...
  stage {
    ...
    configuration {
      ProjectName = "${aws_codebuild_project.cbuild_project_angular.name}"
      }
    }
  }
